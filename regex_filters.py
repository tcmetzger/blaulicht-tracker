...
def includes_media(document):
    ...
    if 'media' in document.keys():
        if 'image' in document['media'].keys():
            ...
            stockpic_titles = re.compile(r'(Blitzerradar)|(Blitzermeldung)|(Tag der offenen Tür)|(Bürgerberatung)|(Infoveranstaltung)|(Einbruchradar)|(Einbruchsradar)|(Wohnungseinbruchradar)|(Fachtagung)|(Ehrung)|(Ehrenabteilung)|(Jahresdienstbesprechung)|(Geschwindigkeitskontrollen)|(Geschwindigkeitsmessungen der)|(?=.*\bGeschwindigkeitsmessung)(?=.*\bKW).*$')
